    ...
    this._deliberationService
      ...
      .subscribe({
        next: (data: any) => {
          Swal.fire({
            title: "Félicitations",
            text: "Délibération terminée avec succès. Merci de vérifier les résultats avant confirmation.",
            icon: "success",
          }).then((_) => {
            this.deliberations.push(data);
            this.deliberations = [...this.deliberations];
            this.deliberations?.forEach((del) => {
              this.classLevelSemester.semesters.forEach((sem) => {
                if (sem.id === del.semester_id) {
                  sem.created = true;
