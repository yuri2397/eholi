            ...
            <div class="row hidden-md-up">
                ...
                <div class="col-md-4 my-2 p-3 border">
                    ...
                    <div class="d-flex px-3 align-items-end ">
                        <img class="cart-img border border-1" src="{{ $appUrl }}/images/logo.png" width="110">
                        <div class="cart-name mx-2  text-start">
                            <h5 class="bold text-capitalize">
                                {{ $student->first_name }} {{ $student->last_name }}
                            </h5>
                            <span class="lead">N° dossier: <span class="bold text-uppercase">{{ $student->reference }}</span></span>
                        </div>
                    </div>
                    <div class="d-flex mx-3 align-item-center justify-content-between">
                        <div>
                            <p class="card-text p-y-1 h6">Né(e): <span class="bold">{{ $student->birth_at }}</span></p>
                            <p class="card-text p-y-1 h6">Adresse: <span class="bold">{{ $student->adress }}</span></p>
                            <p class="card-text p-y-1 h6">Classe: <span class="bold">{{ $class_level->name }}</span></p>
                        </div>
                        <div class="text-center">
                            <span class="lead">Le directeur</span>
                            <br><br>
                            {!! $generator->getBarcode($student->reference, $generator::TYPE_CODE_128) !!}
                        </div>
                    </div>
                </div>
            </div>
